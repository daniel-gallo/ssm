...
def resolve_scan_type(scan_type) -> common.ScanType:
...
def lru_linear_scan(
    ...
    h0 = complex_lib.zeros_like(x,  acc_dtype, x.shape[:1] + x.shape[2:]) if h0 is None else h0
...
def lru_associative_scan(
    ...
    if h0 is not None:
        if reverse:
            a = complex_lib.concatenate(
                [a, complex_lib.ones_like(a[:, :1])], axis=1
            )
            x = complex_lib.concatenate([x, h0[:, None]], axis=1)
        else:
            a = complex_lib.concatenate(
                [complex_lib.ones_like(a[:, :1]), a], axis=1
            )
    ...
    match resolve_scan_type(scan_type):
        case common.ScanType.LINEAR_PALLAS:
            ...
            if acc_float_dtype != jnp.float32:
                raise ValueError(
                    f"Unsupported accumulation dtype: {acc_float_dtype}."
                )
...
def linear_scan(
    ...
    if x.shape[1] == 1:
        ...
        logging.info(
            "Running RNN scan on a single shard in mode %s.", scan_type
        )
