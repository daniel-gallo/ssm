...
from models.recurrentgemma import common
from models.recurrentgemma.jax import array_typing as at
...
class Sampler(Generic[Cache]):
    ...
    def _sample_step(
        ...
        next_token, rng = self._sample_from_logits(
            sampler_state.rng, logits[:, 0]
        )
    ...
    def tokenize(self, input_string: str) -> jax.Array:
        ...
        input_ids = jnp.array(
            [self.vocab.bos_id()] + input_ids, dtype=jnp.int32
        )
        return input_ids
    ...
    def _sample_fn(
        ...
        def sample_with_params(sampler_state: SamplingState) -> SamplingState:
            return self._sample_step(
                params, sampler_state, end_sampling_at_eos_token
            )
    ...
    def _prompt_processing_fn(
        ...
        positions = jnp.repeat(
            jnp.arange(prompt_length)[None], batch_size, axis=0
        )
    ...
    def _get_padded_tokens(
        ...
        pad_values = [max_input_length - len(input_ids) for input_ids in tokens]
    ...
    def __call__(
        ...
        if not self.deterministic_sampling and rng is None:
            raise ValueError(
                "rng must be provided if sampling non-deterministically."
            )
        if total_generation_steps < 0:
