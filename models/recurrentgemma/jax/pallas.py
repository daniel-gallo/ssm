...
from models.recurrentgemma.jax import complex_lib
...
def multi_shard_correction(
    *,
    ...
    if shift_a_prod:
        ...
        a_prod: complex_lib.RealOrComplex = complex_lib.concatenate(
            a_prod, axis=1
        )
...
def get_num_seq_shards(
    ...
    if seq_axis is None:
        ...
        return jax.lax.psum(
            1, seq_axis, axis_index_groups=seq_axis_index_groups
        )
...
def reverse_block_spec(spec: Spec, num_seq_blocks: int) -> Spec:
    ...
    if spec is None:
        ...
        return pl.BlockSpec(
            ...
            index_map=lambda b, d, s: spec.index_map(
                b, d, num_seq_blocks - 1 - s
            ),
        )
...
def linear_rnn_loop_body(
    ...
    h_and_a_prod_carry_refs: tuple[
        complex_lib.RealOrComplex, complex_lib.RealOrComplex | None
    ],
